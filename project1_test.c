...
void SRT(process** processes, int n, int t_cs, float* sum_wait_time, float* sum_turnaround_time, int* context_swtiches, int* preemptions) { // Shortest Remaining Time Algorithm
	int i, real_t = 0, change = 0, terminated = 0, context_switching = 0, ready_capacity = 0, wait_capacity = 0;
	process** ready_queue = createQueue(n);
	process** wait_array = createQueue(n);
	process* CPU = NULL;
	printf("\ntime %dms: Simulator started for SRT ",real_t);
	fflush(stdout);
	printQueue(&ready_queue,ready_capacity);
	while (terminated < n) {
		for (i = 0; i < n - terminated; i++) { // loop for processes leaving the CPU and getting blocked
...
int main(int argc, char const *argv[]) {
	...
	SRT(&processes,n,t_cs,&sum_wait_time[1],&sum_turnaround_time[1],&context_switches[1],&preemptions[2]);
	RR(t_slice, rr_add);
